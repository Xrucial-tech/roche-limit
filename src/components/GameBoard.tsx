...
export const GameBoard: React.FC<Props> = ({ gameState, onSelectPlanet }) => {
  ...
  return (
    ...
    >
      ...
      {gameState.isMerged ? (
          ...
          gameState.stars.map(star => (
            <g key={star.id}>
              {/* Feeding Tendril */}
              <line 
                x1={alpha.position.x} y1={alpha.position.y} 
                x2={beta.position.x} y2={beta.position.y} 
                stroke="#6366f1" 
                strokeWidth={Math.max(0, 30 - starDist/15)} 
                opacity={0.5} 
                filter="url(#glow)"
              />
      ...
      {/* Explosions - Rendered last to be on top */}
